...
int SuperviseThreads(int numObs, int numRegions, bool chatty ){
	...
	FILE* pagesOut;
	pagesOut = dataSaver("LongSkinny.txt");
...
//	printf("superviseThreads5");
	...
	long pageW, pageX, pageY1, pageY2, pageZ;
	pageZ = (long) &i / PAGESIZE;
	pageW = (long) &TotalRevenue / PAGESIZE;
	fprintf(pagesOut, " %ld\n", pageZ);
	fprintf(pagesOut, " %ld\n", pageW);

	for (i=0; i<totalSize; i++){
		TotalRevenue += corporate.Revenue[i];
		pageX = ((long) &corporate.Revenue[i]) / PAGESIZE;
		fprintf(pagesOut, " %ld\n", pageZ);
		fprintf(pagesOut, " %ld\n", pageW);
		fprintf(pagesOut, " %ld\n", pageX);
	}
	...
	for (i=0; i< totalSize;i++){
		...
		pageY1 = ((long) &corporate.Revenue[i]) / PAGESIZE;
		pageY2 = ((long) &corporate.Overhead[i]) / PAGESIZE;
